    ...
    padDecoder(this, true)
{
    if (dabModus == AudioServiceComponentType::DAB) {
        ...
        dablast_push_sink(decoder.get());
    }
    else if (dabModus == AudioServiceComponentType::DABPlus) {
        decoder = std::make_unique<SuperframeFilter>(this, false, false);
        dablast_push_sink(decoder.get());
    }
    else
...
void DecoderAdapter::addtoFrame(uint8_t *v)
{
    ...
    // thread_local: one buffer per DabAudio thread; resize is a no-op after
    // the first call (bitrate is constant for the lifetime of the service)
    thread_local static std::vector<uint8_t> data;
    data.resize(length);

    // Pack 8 consecutive softbits (LSB of each byte) into one output byte.
    // Unrolled so the compiler can auto-vectorise across iterations of i.
    for (size_t i = 0; i < length; i++) {
        const uint8_t* s = v + 8 * i;
        data[i] = (uint8_t)(
            ((s[0] & 1) << 7) | ((s[1] & 1) << 6) |
            ((s[2] & 1) << 5) | ((s[3] & 1) << 4) |
            ((s[4] & 1) << 3) | ((s[5] & 1) << 2) |
...
void DecoderAdapter::ProcessPAD(const uint8_t *xpad_data, size_t xpad_len,
                                 bool exact_xpad_len, const uint8_t *fpad_data)
{
    // MOT (slideshow) is disabled: SetMOTAppType was never called so
    // mot_app_type = -1 and that branch never executes.
    // DLS (Dynamic Label Segment) still runs so onNewDynamicLabel fires.
