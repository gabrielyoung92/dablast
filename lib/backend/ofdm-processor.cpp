...
OFDMProcessor::OFDMProcessor(
    ...
    MscHandler &msc,
    FicHandler &fic,
    RadioReceiverOptions rro) : receiver_options(rro),
                                radioInterface(ri),
                                ...
                                nullSymbolBuffer(params.T_null),
...
void OFDMProcessor::run()
{
    ...
    SyncOnPhase:
        ...
        PROFILE(DecodeTII);
        // The NULL symbol must be drained to stay in frame sync; it also
        // carries TII when enabled.
        getSamples(nullSymbolBuffer.data(), T_null, coarseCorrector + fineCorrector);
        if (rro.decodeTII)
        {
            tiiDecoder.pushSymbols(nullSymbolBuffer, prs);
        }

        PROFILE(OnNewNull);
        radioInterface.onNewNullSymbol({});
        ...
        else if (fineCorrector < -params.carrierDiff / 2)
        {
            coarseCorrector -= params.carrierDiff;
            fineCorrector += params.carrierDiff;
        }
