...
void SuperframeFilter::ProcessUntouchedStream(const uint8_t *data, size_t len) {
	...
	// AudioSpecificConfig() - explicit SBR/PS signalling via top-level objectType.
	// Backward-compatible extension bits cannot be used here: in LATM the ASC
	// has no length boundary, so extension bits corrupt the frameLengthType field
	// that follows immediately. objectType=5 (SBR) or 29 (PS, implies SBR) is
	// the correct form for LATM inline ASC.
	if(sf_format.IsSBR()) {
		au_bw.AddBits(sf_format.ps_flag ? 0b11101 : 0b00101, 5);	// PS(29) or SBR(5)
		au_bw.AddBits(sf_format.GetCoreSrIndex(), 4);		// samplingFrequencyIndex (core)
		...
		au_bw.AddBits(0b00010, 5);							// inner objectType = AAC-LC
		au_bw.AddBits(0b100, 3);							// GASpecificConfig (960-sample transform)
	} else {
