...
CRTL_SDR::CRTL_SDR(RadioControllerInterface& radioController, int deviceIndex) :
...
void CRTL_SDR::open_device()
{
    ...
    if ((uint32_t)deviceIndex >= deviceCount) {
        std::clog << "RTL_SDR: Device index " << deviceIndex
                  << " out of range (found " << deviceCount << " device(s))" << std::endl;
        throw 0;
    }

    std::clog << "RTL_SDR: Opening device " << deviceIndex
              << " of " << deviceCount << std::endl;

    ret = rtlsdr_open(&device, deviceIndex);
    if (ret < 0) {
        std::clog << "RTL_SDR: Opening device " << deviceIndex << " failed" << std::endl;
        throw 0;
    }
