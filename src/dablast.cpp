...
// ── Timestamp streambuf ───────────────────────────────────────────────────────
// Wraps any streambuf and prepends [HH:MM:SS] at the start of every new line.
// Installed on both std::cerr and std::clog in main() so that ALL program
// output (including backend library messages) is timestamped without touching
// individual print statements.
class TimestampBuf : public streambuf
{
public:
    explicit TimestampBuf(streambuf *sink) : sink_(sink), new_line_(true) {}
                     ...
                     const uint8_t *payload, size_t plen)
{
    ...
    else
    {
        ...
        if (stuff > 2)
        {
            memset(buf + 6, 0xFF, stuff - 2);
        } // stuffing
        if (payload && plen > 0)
            memcpy(buf + 4 + stuff, payload, plen);
    }
    cc = (cc + 1) & 0x0F;
}
                   ...
                   const vector<uint8_t> &sec)
{
    ...
    if (1 + copy < 184)
        memset(pl + 1 + copy, 0xFF, 183 - copy);
...
static vector<uint8_t> build_pat(uint16_t pmt_pid)
{
    ...
    s.push_back(0xB0);
    s.push_back(0x0D); // section_length=13
    s.push_back(0x00);
    s.push_back(0x01); // transport_stream_id=1
    s.push_back(0xC1); // version=0, current_next=1
    s.push_back(0x00);
    s.push_back(0x00); // section/last_section_number
    // program 1 → PMT PID
    s.push_back(0x00);
    s.push_back(0x01);
    ...
    s.push_back((crc >> 24) & 0xFF);
    s.push_back((crc >> 16) & 0xFF);
    s.push_back((crc >> 8) & 0xFF);
    s.push_back(crc & 0xFF);
    return s;
}
                                 ...
                                 uint16_t audio_pid, uint8_t stream_type)
{
    ...
    s.push_back((uint8_t)(prog >> 8));
    s.push_back((uint8_t)(prog & 0xFF));
    s.push_back(0xC1);
    s.push_back(0x00);
    s.push_back(0x00);
    ...
    s.push_back(0xF0);
    s.push_back((uint8_t)es_desc.size()); // ES_info_length
...
static vector<uint8_t> build_sdt(const string &service_name)
{
    ...
    s.push_back((uint8_t)(sec_len & 0xFF));
    s.push_back((uint8_t)(DVB_TS_ID >> 8));
    s.push_back((uint8_t)(DVB_TS_ID & 0xFF));
    ...
    s.push_back((uint8_t)(DVB_NW_ID >> 8));
    s.push_back((uint8_t)(DVB_NW_ID & 0xFF));
    s.push_back(0xFF); // reserved_future_use
    // service loop entry
    s.push_back((uint8_t)(DVB_PROG >> 8));
    s.push_back((uint8_t)(DVB_PROG & 0xFF));
...
static vector<uint8_t> build_eit(uint8_t version, const string &event_text)
{
    ...
    // event entry
    s.push_back(0x00);
    s.push_back(0x01); // event_id = 1
    uint8_t mjd[5];
    encode_mjd_utc(mjd);
    s.insert(s.end(), mjd, mjd + 5); // start_time (current UTC)
    s.push_back(0x00);
    s.push_back(0x00);
    s.push_back(0x00); // duration unknown
...
static vector<StreamConfig> parse_config(const string &path)
{
    ...
    while (getline(f, line))
    {
        ...
        try
        {
            c.port = (uint16_t)stoul(addr_port.substr(colon + 1));
        }
        catch (...)
        {
            continue;
        ...
        try
        {
            c.sid = (uint32_t)stoul(sid_str, nullptr, 0);
        }
        catch (...)
        {
            continue;
...
public:
    ...
    void onSignalPresence(bool isSignal) override
    {
        ...
        {
            signal_lost = true;
            cv.notify_all(); // wake do_scan() immediately instead of waiting 30 s
        }
    }
...
private:
    ...
    void send_pes(const uint8_t *data, size_t dlen, uint64_t pts)
    {
...

        {
            ...
            if (extra_stuff > 0)
                memset(pkt + 12, 0xFF, extra_stuff);
...
{
    cerr << "dablast - DAB multicast streamer\n"
            "\n"
            "Usage:\n"
            "  "
...
static vector<long> parse_freq_file(const string &path)
{
    ...
    if (!f)
    {
        cerr << "Cannot open frequency file: " << path << "\n";
        return freqs;
    }
    string line;
    while (getline(f, line))
    {
        ...
        try
        {
            freqs.push_back(strtol(line.c_str(), nullptr, 10));
        }
        catch (...)
        {
            cerr << "Ignoring bad frequency line: " << line << "\n";
        }
    }
    return freqs;
}
...
{
    // Install timestamp prefix on all diagnostic output
    static TimestampBuf ts_cerr(std::cerr.rdbuf());
    static TimestampBuf ts_clog(std::clog.rdbuf());
    std::cerr.rdbuf(&ts_cerr);
    std::clog.rdbuf(&ts_clog);
    ...
    while ((opt = getopt_long(argc, argv, "f:a:g:F:c:l:hS", long_opts, &longidx)) != -1)
    {
        ...
        {
        case 'f':
            freq_hz = strtol(optarg, nullptr, 10);
            break;
        case 'a':
            adapter_index = atoi(optarg);
            break;
        case 'g':
            gain = atoi(optarg);
            break;
